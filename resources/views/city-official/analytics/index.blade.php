...
<div class="space-y-6">
    <div>
        ...
        <p class="text-sm text-slate-500">City analytics with the same layout and breakdown style as barangay analytics.</p>
    </div>
    ...
    <div class="bg-white rounded-lg p-6" style="border: 1px solid #B2BEB5;">
        ...
        <div class="space-y-3">
            @forelse($byBarangay->take(10) as $barangay => $data)
                @php
                    $percentage = $stats['total_projects'] > 0 ? round(($data['count'] / $stats['total_projects']) * 100) : 0;
                @endphp
                <div>
                    <div class="flex justify-between text-sm text-slate-700 mb-1">
                        <span>{{ $barangay }}</span>
                        <span class="font-semibold">₱{{ number_format($data['budget'], 0) }}</span>
                    </div>
                    <div class="h-3 rounded-full bg-slate-100 overflow-hidden">
                        <div class="h-full rounded-full bg-slate-700" style="width: {{ $percentage }}%"></div>
                    </div>
                </div>
            @empty
                <div class="rounded-[20px] border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
                    No barangay budget data available yet.
                </div>
            @endforelse
        </div>
    </div>
    ...
    @if($stats['total_projects'] === 0)
        <div class="bg-blue-50 border border-blue-300 rounded-lg p-4 text-sm text-blue-700">
            No city projects have been recorded yet. Please check back after departments add projects.
        </div>
    @endif
</div>
@endsection