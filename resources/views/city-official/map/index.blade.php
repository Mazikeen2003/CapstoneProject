...
.cm-status-proposed { background: rgba(251,191,36,0.06); color: #d97706; border: 1px solid rgba(251,191,36,0.08); }
.cm-status-proposed .cm-status-dot { background: #f59e0b; box-shadow: 0 0 0 2px rgba(245,158,11,0.12); }
.cm-status-bidding { background: rgba(250,204,21,0.06); color: #a16207; border: 1px solid rgba(250,204,21,0.08); }
.cm-status-bidding .cm-status-dot { background: #eab308; }
.cm-status-ongoing { background: rgba(59,130,246,0.06); color: #2563eb; border: 1px solid rgba(59,130,246,0.08); }
.cm-status-ongoing .cm-status-dot { background: #3b82f6; box-shadow: 0 0 0 2px rgba(59,130,246,0.12); }
.cm-status-award { background: rgba(139,92,246,0.06); color: #7c3aed; border: 1px solid rgba(139,92,246,0.08); }
.cm-status-award .cm-status-dot { background: #8b5cf6; }
.cm-status-implementation { background: rgba(14,165,233,0.06); color: #0284c7; border: 1px solid rgba(14,165,233,0.08); }
.cm-status-implementation .cm-status-dot { background: #0ea5e9; }
.cm-status-completed { background: rgba(16,185,129,0.06); color: #059669; border: 1px solid rgba(16,185,129,0.08); }
.cm-status-completed .cm-status-dot { background: #10b981; box-shadow: 0 0 0 2px rgba(16,185,129,0.12); }
.cm-status-hold { background: rgba(239,68,68,0.06); color: #dc2626; border: 1px solid rgba(239,68,68,0.08); }
.cm-status-hold .cm-status-dot { background: #ef4444; }
.cm-status-cancelled { background: rgba(107,114,128,0.06); color: #6b7280; border: 1px solid rgba(107,114,128,0.08); }
.cm-status-cancelled .cm-status-dot { background: #9ca3af; }
.dark .cm-status-proposed { background: rgba(251,191,36,0.05); color: #fbbf24; }
.dark .cm-status-bidding { background: rgba(250,204,21,0.05); color: #facc15; }
.dark .cm-status-ongoing { background: rgba(59,130,246,0.05); color: #60a5fa; }
.dark .cm-status-award { background: rgba(139,92,246,0.05); color: #a78bfa; }
.dark .cm-status-implementation { background: rgba(14,165,233,0.05); color: #38bdf8; }
.dark .cm-status-completed { background: rgba(16,185,129,0.05); color: #34d399; }
.dark .cm-status-hold { background: rgba(239,68,68,0.05); color: #f87171; }
.dark .cm-status-cancelled { background: rgba(148,163,184,0.05); color: #94a3b8; }
...
.cm-step-dot {
    ...
    font-size: 0.75rem;
    font-weight: 800;
    border: 3px solid var(--cm-line-strong);
    background: var(--cm-surface);
    color: var(--cm-muted);
    transition: var(--cm-transition);
}
.cm-step.complete .cm-step-dot {
    border-color: #10b981;
    background: linear-gradient(135deg, #10b981, #059669);
    color: #ffffff;
    box-shadow: 0 4px 12px -2px rgba(16,185,129,0.3);
}
.cm-step.current .cm-step-dot {
    border-color: #3b82f6;
    background: linear-gradient(135deg, #3b82f6, #2563eb);
    color: #ffffff;
    box-shadow: 0 4px 12px -2px rgba(59,130,246,0.3);
    animation: cmStepPulse 2s ease-in-out infinite;
}
...
.cm-step.complete .cm-step-label { color: #059669; }
.cm-step.current .cm-step-label { color: var(--cm-indigo); }
.cm-step-line {
    flex: 1;
    height: 3px;
    margin-top: 15px;
...
.cm-step-line.complete { background: linear-gradient(90deg, #10b981, #059669); }
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function getStatusClass(status) {
            const map = {
                'Proposed': 'cm-status-proposed', 'Planning': 'cm-status-proposed',
                'For bidding': 'cm-status-bidding', 'Procurement': 'cm-status-bidding',
                'Bidding ongoing': 'cm-status-ongoing', 'Bidding - Success': 'cm-status-award',
                'Award of contract': 'cm-status-award', 'Implementation': 'cm-status-implementation',
                'On Going': 'cm-status-implementation', 'Completed': 'cm-status-completed',
                'On Hold': 'cm-status-hold', 'Cancelled': 'cm-status-cancelled'
            };
        ...
        function renderLifecycleStepper(status) {
            ...
            return `<div class="cm-lifecycle"><div class="cm-lifecycle-head"><span class="cm-lifecycle-title">Project lifecycle</span><span class="cm-lifecycle-status">${status || 'Unknown'}</span></div><div class="cm-lifecycle-track">${steps.map((step, stepIndex) => { const complete = activeStep !== undefined && (stepIndex < activeStep || completedProject); const current = activeStep !== undefined && stepIndex === activeStep && !completedProject; const state = complete ? 'complete' : (current ? 'current' : ''); const line = stepIndex < steps.length - 1 ? `<div class="cm-step-line ${complete ? 'complete' : ''}"></div>` : ''; return `<div class="cm-step ${state}"><div class="cm-step-dot">${complete ? '&#10003;' : stepIndex + 1}</div><span class="cm-step-label">${step}</span></div>${line}`; }).join('')}</div></div>`;
        }
