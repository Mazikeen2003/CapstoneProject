...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
    <div class="admin-dashboard-hero rounded-3xl px-6 py-7 shadow-lg sm:px-8">
        <div>
            <p class="text-xs font-bold uppercase tracking-[0.24em] text-amber-300">City operations workspace</p>
            <h1 class="mt-2 text-3xl font-bold tracking-tight text-white sm:text-4xl">City Official Dashboard</h1>
            <p class="mt-2 max-w-2xl text-sm leading-6 text-slate-300">A focused view of city projects, locations, and current delivery progress.</p>
        </div>
    </div>
    ...
    <div class="admin-dashboard-activity overflow-hidden rounded-2xl shadow-sm">
        <div class="admin-card-header border-b border-slate-200 px-6 py-5">
            ...
            <p class="mt-1 text-sm text-slate-500">Explore the geographic distribution of city projects.</p>
        </div>
        <div class="p-6">
            <div id="city-map" class="relative z-0 h-[42vh] overflow-hidden rounded-3xl border border-slate-200 sm:h-[48vh] md:h-[56vh]"></div>
        </div>
    </div>
    ...
    <div class="admin-dashboard-activity overflow-hidden rounded-2xl shadow-sm">
        <div class="admin-card-header flex flex-col gap-2 border-b border-slate-200 px-6 py-5 sm:flex-row sm:items-end sm:justify-between">
            <div>
                ...
                <p class="text-sm text-slate-500">Latest city project activity.</p>
            </div>
        </div>
                    ...
                    @foreach ($recentProjects as $project)
                        <div class="rounded-3xl border border-slate-200 bg-slate-50 p-4 shadow-sm">
                            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                ...
                                <a href="{{ route('city.projects.show', $project->project_id) }}" class="inline-flex rounded-full border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-900 hover:bg-slate-100">View</a>
                            </div>
                        </div>
                    @endforeach
                </div>
