...
class AnalyticsController extends Controller
{
    public function index(Request $request)
    {
        ...
        $byBarangay = $projects->groupBy(fn ($project) => $project->barangay?->barangay_name ?? 'Citywide')
            ->map(fn ($group) => [
                'count' => $group->count(),
                'budget' => $group->sum('approved_budget'),
            ])
        ...
        return view('engineering.analytics.index', compact('stats', 'byStatus', 'byBarangay', 'availableYears', 'statusYear', 'budgetYear', 'budgetStats', 'insights'));
    }
}
